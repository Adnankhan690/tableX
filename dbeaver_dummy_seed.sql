-- tableX local dummy seed for DBeaver.
--
-- Run this only after the migrations have been applied.
-- It inserts TWO demo restaurants:
--   Spice Garden  (spice-garden)  -- 3 staff, 8 tables, 7 categories, 28 items, 5% GST
--   Coastal Curry (coastal-curry) -- 2 staff, 4 tables, 4 categories, 14 items, 5% GST + 10% service
--
-- Two restaurants rather than one on purpose: with a single tenant every query returns the right
-- rows by accident, so nothing proves the restaurant_id scoping actually works.
